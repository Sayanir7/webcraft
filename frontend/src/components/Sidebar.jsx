...
const Sidebar = () => {
  const { chats, setChats, error,loading } = useFetchChatTitles();
    ...
    <div className="relative group">
      <div className="absolute top-0 left-0 h-full bg-primary text-primary_text transition-all duration-300 transform -translate-x-full group-hover:translate-x-0 opacity-0 group-hover:opacity-100 z-10 w-64 overflow-y-auto">
        <div className="p-4">
          <Link
            to="/new"
            className="flex items-center bg-accent hover:bg-hover_accent p-2 rounded-md mb-4"
          >
          ...
          />
          {loading && (
            <div className="text-red-200 text-sm w-full text-center text-italic">
              <LoadingSpinner/>
            </div>
          )}
          {!error ? (
            ...
            <div className="text-red-200 text-sm w-full text-center text-italic">{error}!</div>
          )}
        </div>
      </div>
